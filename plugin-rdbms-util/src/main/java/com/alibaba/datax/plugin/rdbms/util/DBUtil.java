...
//                    throw new Exception(DBUtilErrorCode.JDBC_NULL.toString());
     ...
     */
    private static boolean isSlaveBehind(Connection conn,DataBaseType dataBaseType) {
        try {
            ResultSet rs = query(conn, "SHOW VARIABLES LIKE 'read_only'",dataBaseType);
            if (DBUtil.asyncResultSetNext(rs)) {
                String readOnly = rs.getString("Value");
                if ("ON".equalsIgnoreCase(readOnly)) { //备库
                    ResultSet rs1 = query(conn, "SHOW SLAVE STATUS",dataBaseType);
                    if (DBUtil.asyncResultSetNext(rs1)) {
                        ...
                        if ("Yes".equalsIgnoreCase(ioRunning) && "Yes".equalsIgnoreCase(sqlRunning)) {
                            ResultSet rs2 = query(conn, "SELECT TIMESTAMPDIFF(SECOND, CURDATE(), NOW())",dataBaseType);
    ...
    public static boolean hasInsertPrivilege(DataBaseType dataBaseType, String jdbcURL, String userName, String password, List<String> tableList) {
        ...
        try {
            ResultSet rs = query(connection, "SHOW GRANTS FOR " + userName,dataBaseType);
     ...
     */
    public static ResultSet query(Connection conn, String sql, int fetchSize,DataBaseType dataBaseType)
            throws SQLException {
        // 默认3600 s 的query Timeout
        return query(conn, sql, fetchSize, Constant.SOCKET_TIMEOUT_INSECOND,dataBaseType);
    }
     ...
     * @throws SQLException
     */
    public static ResultSet query(Connection conn, String sql, int fetchSize, int queryTimeout,DataBaseType dataBaseType)
            throws SQLException {
        ...
        if(!DataBaseType.HIVE.equals(dataBaseType)){
        ...
        try {
            ...
            if (connection != null) {
                if (dataBaseType.equals(DataBaseType.MySql) && checkSlave) {
                    //dataBaseType.MySql
                    boolean connOk = !isSlaveBehind(connection,dataBaseType);
                    return connOk;
                } else {
                    return true;
                }
        ...
        try {
            ...
            if (null != connection) {
                for (String pre : preSql) {
                    if (doPreCheck(connection, pre,dataBaseType) == false) {
            ...
            return RetryUtil.executeWithRetry(new Callable<Boolean>() {
                ...
                public Boolean call() throws Exception {
                    ...
                    try {
                        ...
                        ResultSet rs = query(conn, "select DATABASE_ROLE from V$DATABASE",DataBaseType.Oracle);
    ...
    public static ResultSet query(Connection conn, String sql,DataBaseType dataBaseType)
            throws SQLException {
        Statement stmt = conn.createStatement(ResultSet.TYPE_FORWARD_ONLY,
                ResultSet.CONCUR_READ_ONLY);
        //默认3600 seconds
    ...
    private static boolean doPreCheck(Connection conn, String pre,DataBaseType dataBaseType) {
        ResultSet rs = null;
        try {
            rs = query(conn, pre,dataBaseType);
