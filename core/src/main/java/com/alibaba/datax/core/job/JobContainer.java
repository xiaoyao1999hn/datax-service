...
//                }
    ...
    private Reader.Job preCheckReaderInit(JobPluginCollector jobPluginCollector) {
        ...
        classLoaderSwapper.setCurrentThreadClassLoader(LoadUtil.getJarLoader(jobId,
                PluginType.READER, this.readerPluginName));

        Reader.Job jobReader = (Reader.Job) LoadUtil.loadJobPlugin(jobId,
                PluginType.READER, this.readerPluginName);
    ...
    private Writer.Job preCheckWriterInit(JobPluginCollector jobPluginCollector) {
        ...
        Writer.Job jobWriter = (Writer.Job) LoadUtil.loadJobPlugin(jobId,
                PluginType.WRITER, this.writerPluginName);
    ...
    private void preHandle() {
        ...
        AbstractJobPlugin handler = LoadUtil.loadJobPlugin(jobId,
                handlerPluginType, handlerPluginName);
    ...
    private void logStatistics() {
        ...
        Pair<DataXJob,DataXReport> dataXJob=DataXJobManager.INSTANCE.getJob(configuration.getLong(CoreConstant.DATAX_CORE_CONTAINER_JOB_ID));
        dataXJob.getValue().setRunTimes(String.valueOf(totalCosts)+ "s");
        dataXJob.getValue().setAvgFlow( StrUtil.stringify(byteSpeedPerSecond)+ "/s");
        dataXJob.getValue().setSpeed(String.valueOf(recordSpeedPerSecond) + "rec/s");
        dataXJob.getValue().setTotalFailRecordCount(String.valueOf(CommunicationTool.getTotalErrorRecords(communication)));
        dataXJob.getValue().setTotalRecordCount( String.valueOf(CommunicationTool.getTotalReadRecords(communication)));
        //todo 这里有bug写入总数总是比读出多1
        dataXJob.getValue().setTotalWriteRecordCount(String.valueOf(CommunicationTool.getTotalReadRecords(communication)));
        dataXJob.getValue().setTotalReadBytes(BytesUtil.getBytes(CommunicationTool.getTotalReadBytes(communication)));
        dataXJob.getValue().setTotalWriteBytes(BytesUtil.getBytes(CommunicationTool.getTotalReadBytes(communication)));
