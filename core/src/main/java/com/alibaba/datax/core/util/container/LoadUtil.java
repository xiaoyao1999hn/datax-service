...
//    private static ThreadLocal<Configuration> pluginRegisterCenter=new ThreadLocal();

    private static Map<Long,Configuration> pluginRegisterCenter=new ConcurrentHashMap<>();
    ...
    public static void bind(Configuration pluginConfigs) {
        pluginRegisterCenter.putIfAbsent(pluginConfigs.getLong(CoreConstant.DATAX_CORE_CONTAINER_JOB_ID),pluginConfigs);
    }
    ...
    private static Configuration getPluginConf(Long jobId,PluginType pluginType,
                                               String pluginName) {
        Configuration pluginConf = pluginRegisterCenter.get(jobId)
     ...
     * @return
     */
    public static AbstractJobPlugin loadJobPlugin(Long jobId,PluginType pluginType,
                                                  String pluginName) {
        Class<? extends AbstractPlugin> clazz = LoadUtil.loadPluginClass(jobId,
                pluginType, pluginName, ContainerType.Job);

        try {
            AbstractJobPlugin jobPlugin = (AbstractJobPlugin) clazz
                    .newInstance();
            jobPlugin.setPluginConf(getPluginConf(jobId,pluginType, pluginName));
            return jobPlugin;
        } catch (Exception e) {
            throw DataXException.asDataXException(
     ...
     * @return
     */
    public static AbstractTaskPlugin loadTaskPlugin(Long jobId,PluginType pluginType,
                                                    String pluginName) {
        Class<? extends AbstractPlugin> clazz = LoadUtil.loadPluginClass(jobId,
        ...
        try {
            AbstractTaskPlugin taskPlugin = (AbstractTaskPlugin) clazz
                    .newInstance();
            taskPlugin.setPluginConf(getPluginConf(jobId,pluginType, pluginName));
            return taskPlugin;
        } catch (Exception e) {
     ...
     * @return
     */
    public static AbstractRunner loadPluginRunner(Long jobId,PluginType pluginType, String pluginName) {
        AbstractTaskPlugin taskPlugin = LoadUtil.loadTaskPlugin(jobId,pluginType,
                pluginName);
    ...
    private static synchronized Class<? extends AbstractPlugin> loadPluginClass(Long jobId,
            PluginType pluginType, String pluginName,
            ContainerType pluginRunType) {
        Configuration pluginConf = getPluginConf(jobId,pluginType, pluginName);
        JarLoader jarLoader = LoadUtil.getJarLoader(jobId,pluginType, pluginName);
        try {
    ...
    public static synchronized JarLoader getJarLoader(Long jobId,PluginType pluginType,
                                                      String pluginName) {
        Configuration pluginConf = getPluginConf(jobId,pluginType, pluginName);
    ...
    public static void  releasePluginConf(Long jobId){
        pluginRegisterCenter.remove(jobId);
    }
}
