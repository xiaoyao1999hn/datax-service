...
public class UnstructuredStorageReaderUtil {
	...
	public static void doReadFromStream(BufferedReader reader, String context,
		...
		try {
			...
			List<ColumnEntry> partitionParam=readerSliceConfig.getList("partitionParam",ColumnEntry.class);

			String[] parseRows;
			while ((parseRows = UnstructuredStorageReaderUtil
					.splitBufferedReader(csvReader)) != null) {
				//补齐分区字段值,hdfs默认是没有分区字段值的
				if(partitionParam!=null){
					//todo  这里需要考虑本身这个文档里面是否存在分区字段，如果存在则替换，如果不存在则加多一个，默认情况下是不存在的
					for (ColumnEntry columnEntry:partitionParam){
						if(parseRows.length<columnEntry.getIndex()){
							String[] tempRows=new String[parseRows.length+1];
							System.arraycopy(parseRows,0,tempRows,0,parseRows.length);
							parseRows=tempRows;
						}
					}

					for (ColumnEntry columnEntry:partitionParam){
						parseRows[columnEntry.getIndex()]=columnEntry.getValue();
					}
				}
		...
		if (null == columnConfigs || columnConfigs.size() == 0) {
			...
			try {
				for (ColumnEntry columnConfig : columnConfigs) {
					...
					columnValue = convertNullFormatToDefaultValue(type, columnValue);
					...
					switch (type) {
						...
						case DECIMAL:
							try {
								columnGenerated = new DecimalColumn(columnValue);
							} catch (Exception e) {
								throw new IllegalArgumentException(String.format(
										"类型转换错误, 无法将[%s] 转换为[%s]", columnValue,
										"DECIMAL"));
	...
	public static String convertNullFormatToDefaultValue(Type columnType, String originVal) {
		String retVal = originVal;
		if (StringUtils.isBlank(originVal) || originVal.contains("\\N") || originVal.contains("\\\\N")) {
			switch (columnType) {
				case STRING:
					retVal = "";
					break;
